 ...
 * content is lost)
 *            @arg @ref PWR_FULL_SRAM2_RETENTION      Full SRAM2 is powered by the low-power regulator
 * in Standby mode
 ...
 * propagated only to the peripheral requesting it. SRAM1, SRAM2 and register contents are preserved.
 ...
 *        (LCD, LPTIM1, I2C3 and LPUART) can switch on the HSI to receive a frame, and switch off the
 * HSI after receiving the frame if it is not a wakeup frame. In this case the HSI clock is propagated
 * only to the peripheral requesting it. SRAM1, SRAM2 and register contents are preserved. SRAM3
 * content is preserved depending on RRSTP bit setting (not available on all devices). The BOR is
 * available. The voltage regulator is set in low-power mode but LPR bit must be cleared to enter stop
 * 2 mode. Otherwise, Stop 1 mode is entered.
