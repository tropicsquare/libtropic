...
static int session3(lt_handle_t *h)
{
    ...
    uint8_t msg[] = {'a', 'h', 'o', 'j'};
    uint8_t rs[64];
    ret = lt_ecc_eddsa_sign(h, ECC_SLOT_0, msg, 4, rs, 64);
    if (LT_OK != ret) {
    ...
    ret = lt_ecc_key_read(h, ECC_SLOT_0, slot_0_pubkey, 64, &curve, &origin);
    if (LT_OK != ret) {
    ...
    ret = lt_random_get(h, buff, RANDOM_VALUE_GET_LEN_MAX);
    if (LT_OK != ret) {
