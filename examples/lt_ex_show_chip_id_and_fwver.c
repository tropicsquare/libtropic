...
static void print_header_boot_v1_0_1(uint8_t *data, bank_id_t bank_id)
{
    ...
    LT_LOG_INFO("      Type:                           %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8, p_h->type[3],
                p_h->type[2], p_h->type[1], p_h->type[0]);
    LT_LOG_INFO("      Version:                        %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8, p_h->version[3],
                p_h->version[2], p_h->version[1], p_h->version[0]);
    LT_LOG_INFO("      Size:                           %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8, p_h->size[3],
                p_h->size[2], p_h->size[1], p_h->size[0]);
    LT_LOG_INFO("      Git hash:                       %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8, p_h->git_hash[3],
                p_h->git_hash[2], p_h->git_hash[1], p_h->git_hash[0]);
    LT_LOG_INFO("      FW hash:                        %02" PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8, p_h->hash[3],
                p_h->hash[2], p_h->hash[1], p_h->hash[0]);
}

static void print_all_headers_v1(lt_handle_t *h)
{
    LT_LOG_INFO("  Reading firmware headers in all banks...");
...
{
    LT_LOG_INFO("\t=======================================================================");
    LT_LOG_INFO("\t=====  TROPIC01 show chip ID and firmware versions                  ===");
    LT_LOG_INFO("\t=======================================================================");
    ...
    LT_LOG_INFO("  Rebooting into APPLICATION mode to check firmware versions...");
    ...
    if (h->l2.mode == LT_MODE_APP) {
        ...
        if (ret == LT_OK) {
            LT_LOG_INFO("  Chip is executing RISCV application firmware version: %02" PRIX8 ".%02" PRIX8 ".%02" PRIX8
                        "    (+ .%02" PRIX8 ")",
        ...
        LT_LOG_ERROR("Device couldn't get into APP mode, APP and SPECT firmwares in fw banks are not valid");
    }
    ...
    if (h->l2.mode == LT_MODE_MAINTENANCE) {
        ...
        if (((fw_ver[3] & 0x7f) == 1) && (fw_ver[2] == 0) && (fw_ver[1] == 1) && (fw_ver[0] == 0)) {
            ...
            if (((fw_ver[3] & 0x7f) == 2) && (fw_ver[2] == 0) && (fw_ver[1] == 1) && (fw_ver[0] == 0)) {
                LT_LOG_INFO("  Bootloader version:                 %02" PRIX8 ".%02" PRIX8 ".%02" PRIX8
                            "    (+ .%02" PRIX8 ")",
                            fw_ver[3] & 0x7f, fw_ver[2], fw_ver[1], fw_ver[0]);
