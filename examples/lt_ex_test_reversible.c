...
void lt_ex_test_reversible(void)
{
    ...
    LOG_OUT_RESULT("%d", lt_get_info_chip_id(&h, chip_id, LT_L2_GET_INFO_CHIP_ID_SIZE));

    LOG_OUT("%s", "lt_get_info_riscv_fw_ver() ");
    uint8_t riscv_fw_ver[LT_L2_GET_INFO_RISCV_FW_SIZE] = {0};
    LOG_OUT_RESULT("%d", lt_get_info_riscv_fw_ver(&h, riscv_fw_ver, LT_L2_GET_INFO_RISCV_FW_SIZE));

    LOG_OUT("%s", "lt_get_info_spect_fw_ver() ");
    uint8_t spect_fw_ver[LT_L2_GET_INFO_SPECT_FW_SIZE] = {0};
    LOG_OUT_RESULT("%d", lt_get_info_spect_fw_ver(&h, spect_fw_ver, LT_L2_GET_INFO_SPECT_FW_SIZE));

    uint8_t X509_cert[LT_L2_GET_INFO_REQ_CERT_SIZE] = {0};
    LOG_OUT("%s", "lt_get_info_cert() ");
    LOG_OUT_RESULT("%d", lt_get_info_cert(&h, X509_cert, LT_L2_GET_INFO_REQ_CERT_SIZE));

    LOG_OUT("%s", "lt_cert_verify_and_parse() ");
    uint8_t stpub[32] = {0};
    LOG_OUT_RESULT("%d", lt_cert_verify_and_parse(X509_cert, LT_L2_GET_INFO_REQ_CERT_SIZE, stpub));

    LOG_OUT("%s", "lt_session_start() ");
    LOG_OUT_RESULT("%d", lt_session_start(&h, stpub, pkey_index_0, sh0priv, sh0pub));
    ...
    LOG_OUT("---------------- Loop through all key slots, ed25519, stored key ----------------");
    ...
    for(uint16_t i=0; i<32; i++) {
        ...
        LOG_OUT_RESULT("%d", lt_ecc_key_store(&h, i, CURVE_ED25519, attestation_key));
        ...
        uint8_t msg[] = {'a', 'h', 'o', 'j'};
        uint8_t rs[64];
        LOG_OUT_RESULT("%d", lt_ecc_eddsa_sign(&h, i, msg, 4, rs, 64));

        LOG_OUT("lt_ecc_eddsa_sig_verify() slot %d  ", i);
        LOG_OUT_RESULT("%d", lt_ecc_eddsa_sig_verify(msg, 4, slot_0_pubkey, rs));

        LOG_OUT("lt_ecc_key_erase() slot        %d  ", i);
        LOG_OUT_RESULT("%d", lt_ecc_key_erase(&h, i));

        LOG_OUT("");
    }

    LOG_OUT("---------------- Loop through all key slots, ed25519, generated key ----------------");

    for(int i=0; i<32; i++) {
        LOG_OUT("lt_ecc_key_generate() slot     %d  ", i);
        LOG_OUT_RESULT("%d", lt_ecc_key_generate(&h, i, CURVE_ED25519));

        LOG_OUT("lt_ecc_key_read() slot         %d  ", i);
        uint8_t slot_0_pubkey[64];
        lt_ecc_curve_type_t curve;
        ecc_key_origin_t origin;
        LOG_OUT_RESULT("%d", lt_ecc_key_read(&h, i, slot_0_pubkey, 64, &curve, &origin));
    ...
    LOG_OUT_RESULT("%d", lt_random_get(&h, buff, RANDOM_VALUE_GET_LEN_MAX));

    LOG_OUT("lt_session_abort()");
    LOG_OUT_RESULT("%d", lt_session_abort(&h));

    LOG_OUT("lt_deinit()");
    LOG_OUT_RESULT("%d", lt_deinit(&h));
