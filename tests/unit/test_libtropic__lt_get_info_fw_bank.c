/**
 * @file test_libtropic__lt_get_info_fw_bank.c
...
// Test if function returns LT_PARAM_ERROR when wrong fw_bank is passed
void test__invalid_fw_bank()
{
    lt_handle_t h;
...
void test__invalid_fw_bank_len()
{
    lt_handle_t h;
    uint8_t fw_bank[LT_L2_GET_INFO_FW_HEADER_SIZE];
...
lt_ret_t callback_1__lt_l2_transfer(lt_handle_t *h, int cmock_num_calls)
{
    ...
    p_l2_rsp->rsp_len = LT_L2_GET_INFO_FW_HEADER_SIZE + 1;
...
lt_ret_t callback_2__lt_l2_transfer(lt_handle_t *h, int cmock_num_calls)
{
    ...
    p_l2_rsp->rsp_len = LT_L2_GET_INFO_FW_HEADER_SIZE;
