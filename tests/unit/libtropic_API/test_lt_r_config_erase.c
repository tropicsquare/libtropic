/**
 * @file test_libtropic__lt_r_config_erase.c
...
void test__invalid_handle() { TEST_ASSERT_EQUAL(LT_PARAM_ERR, lt_r_config_erase(NULL)); }
...
/*void test___no_session()
{
    ...
    TEST_ASSERT_EQUAL(LT_HOST_NO_SESSION, lt_r_config_erase(&h));
}
...
void test___l3_cmd_fail()
{
    ...
    for (size_t i = 0; i < (sizeof(rets)/sizeof(rets[0])); i++) {
        ...
        TEST_ASSERT_EQUAL(rets[i], lt_r_config_erase(&h));
    }
}
...
void test___len_mismatch()
{
    ...
    TEST_ASSERT_EQUAL(LT_FAIL, lt_r_config_erase(&h));
}
...
void test___correct()
{
    ...
    size_inject_value = 1;
    lt_l3_cmd_Stub(callback__lt_l3_cmd);
    TEST_ASSERT_EQUAL(LT_OK, lt_r_config_erase(&h));
}
*/