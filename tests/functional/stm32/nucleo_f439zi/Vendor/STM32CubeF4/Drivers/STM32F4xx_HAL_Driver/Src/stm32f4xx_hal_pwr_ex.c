    ...
    [..]
      (+) By setting the FPDS bit in the PWR_CR register by using the
          ...
          waking up from Stop mode.
           ...
           (+) For STM32F42xxx/43xxx/446xx/469xx/479xx Devices, the scale can be modified only when the
PLL is OFF and the HSI or HSE clock source is selected as system clock. The new value programmed is
active only when the PLL is ON. When the PLL is OFF, the voltage scale 3 is automatically selected.
    ...
    [..]
       (+) For STM32F42xxx/43xxx/446xx/469xx/479xx Devices, in Run mode: the main regulator has
        ...
        (++) Over-drive mode: This mode allows the CPU and the core logic to operate at a
            ...
            disabled by HAL_PWREx_DisableOverDrive() function, to enter or exit from Over-drive mode
 ...
 * devices. This mode allows the CPU and the core logic to operate at a higher frequency than the
