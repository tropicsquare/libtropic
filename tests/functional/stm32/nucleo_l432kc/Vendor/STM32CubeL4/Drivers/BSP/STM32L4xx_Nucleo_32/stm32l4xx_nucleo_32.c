...
#define __STM32L4XX_NUCLEO_32_BSP_VERSION                                                             \
    ((__STM32L4XX_NUCLEO_32_BSP_VERSION_MAIN << 24) |                                                 \
     (__STM32L4XX_NUCLEO_32_BSP_VERSION_SUB1 << 16) | (__STM32L4XX_NUCLEO_32_BSP_VERSION_SUB2 << 8) | \
...
HAL_StatusTypeDef I2C1_WriteBuffer(uint16_t Addr, uint8_t Reg, uint16_t RegSize, uint8_t *pBuffer,
                                   uint16_t Length);
HAL_StatusTypeDef I2C1_ReadBuffer(uint16_t Addr, uint8_t Reg, uint16_t RegSize, uint8_t *pBuffer,
                                  uint16_t Length);
 ...
 */
uint32_t BSP_GetVersion(void) { return __STM32L4XX_NUCLEO_32_BSP_VERSION; }
 ...
 * @retval None
 */
void BSP_LED_On(Led_TypeDef Led) { HAL_GPIO_WritePin(LED_PORT[Led], LED_PIN[Led], GPIO_PIN_SET); }
 ...
 * @retval None
 */
void BSP_LED_Off(Led_TypeDef Led) { HAL_GPIO_WritePin(LED_PORT[Led], LED_PIN[Led], GPIO_PIN_RESET); }
 ...
 * @retval None
 */
void BSP_LED_Toggle(Led_TypeDef Led) { HAL_GPIO_TogglePin(LED_PORT[Led], LED_PIN[Led]); }
...
void I2C1_Write(uint8_t Addr, uint8_t Reg, uint8_t Value)
{
    ...
    status = HAL_I2C_Mem_Write(&nucleo32_I2c1, Addr, (uint16_t)Reg, I2C_MEMADD_SIZE_8BIT, &Value, 1,
                               I2c1Timeout);
...
{
    const uint8_t lookup[10] = {0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F};
