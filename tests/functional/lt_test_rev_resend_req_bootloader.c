/**
 * @file lt_test_rev_resend_req_bootloader.c
 * @brief Test L2 Resend Request in Maintenance mode.
...
lt_ret_t lt_test_rev_resend_req_bootloader_cleanup(void)
{
    lt_ret_t ret;
...
{
    LT_LOG_INFO("-----------------------------------");
    LT_LOG_INFO("lt_test_rev_resend_req_bootloader()");
    LT_LOG_INFO("-----------------------------------");
    ...
    lt_test_cleanup_function = &lt_test_rev_resend_req_bootloader_cleanup;
    ...
    LT_TEST_ASSERT(LT_OK, lt_test_rev_resend_req_bootloader_cleanup());
