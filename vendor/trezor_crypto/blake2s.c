...
int blake2s_InitPersonal(blake2s_state *S, size_t outlen, const void *personal, size_t personal_len)
{
    ...
    if ((!personal) || (personal_len != BLAKE2S_PERSONALBYTES)) {
        return -1;
    }
...
{
    BLAKE2S_CTX ctx;
    if (0 != blake2s_Init(&ctx, outlen)) {
        return -1;
    }
    if (0 != blake2s_Update(&ctx, msg, msg_len)) {
        return -1;
    }
    if (0 != blake2s_Final(&ctx, out, outlen)) {
        return -1;
    }
    return 0;
}

int blake2s_Key(const uint8_t *msg, uint32_t msg_len, const void *key, size_t keylen, void *out,
                size_t outlen)
{
    BLAKE2S_CTX ctx;
    if (0 != blake2s_InitKey(&ctx, outlen, key, keylen)) {
        return -1;
    }
    if (0 != blake2s_Update(&ctx, msg, msg_len)) {
        return -1;
