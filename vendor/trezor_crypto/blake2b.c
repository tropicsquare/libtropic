...
int blake2b_InitPersonal(blake2b_state *S, size_t outlen, const void *personal, size_t personal_len)
{
    ...
    if ((!personal) || (personal_len != BLAKE2B_PERSONALBYTES)) {
        return -1;
    }
...
{
    BLAKE2B_CTX ctx;
    if (0 != blake2b_Init(&ctx, outlen)) {
        return -1;
    }
    if (0 != blake2b_Update(&ctx, msg, msg_len)) {
        return -1;
    }
    if (0 != blake2b_Final(&ctx, out, outlen)) {
        return -1;
    }
    return 0;
}
...
{
    BLAKE2B_CTX ctx;
    if (0 != blake2b_InitKey(&ctx, outlen, key, keylen)) {
        return -1;
    }
    if (0 != blake2b_Update(&ctx, msg, msg_len)) {
        return -1;
