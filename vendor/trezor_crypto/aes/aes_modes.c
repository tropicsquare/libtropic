...
AES_RETURN aes_ecb_encrypt(const unsigned char *ibuf, unsigned char *obuf, int len,
...
AES_RETURN aes_ecb_decrypt(const unsigned char *ibuf, unsigned char *obuf, int len,
...
AES_RETURN aes_cbc_encrypt(const unsigned char *ibuf, unsigned char *obuf, int len, unsigned char *iv,
                           const aes_encrypt_ctx ctx[1])
{
    int nb = len >> AES_BLOCK_SIZE_P2;
...
AES_RETURN aes_cbc_decrypt(const unsigned char *ibuf, unsigned char *obuf, int len, unsigned char *iv,
                           const aes_decrypt_ctx ctx[1])
{
...
AES_RETURN aes_cfb_encrypt(const unsigned char *ibuf, unsigned char *obuf, int len, unsigned char *iv,
...
AES_RETURN aes_cfb_decrypt(const unsigned char *ibuf, unsigned char *obuf, int len, unsigned char *iv,
                           aes_encrypt_ctx ctx[1])
{
    int cnt = 0, b_pos = (int)ctx->inf.b[2], nb = 0;
...
AES_RETURN aes_ofb_crypt(const unsigned char *ibuf, unsigned char *obuf, int len, unsigned char *iv,
                         aes_encrypt_ctx ctx[1])
{
    int cnt = 0, b_pos = (int)ctx->inf.b[2], nb = 0;
...
AES_RETURN aes_ctr_crypt(const unsigned char *ibuf, unsigned char *obuf, int len, unsigned char *cbuf,
                         cbuf_inc ctr_inc, aes_encrypt_ctx ctx[1])
{
...
void aes_ctr_cbuf_inc(unsigned char *cbuf)
{
    ...
    while (i >= 0) {
        cbuf[i]++;
        if (cbuf[i]) {
            return;  // if there was no overflow
        }
        i--;
    }
}
