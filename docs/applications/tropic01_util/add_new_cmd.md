...
Before you start, complete [Install and Use](install_and_use.md) and then follow these steps to install the `protobuf-compiler`, which is needed when adding new Protobuf messages, as the files have to be re-generated:
...
2. Come up with a new name in format `<cmd_name_in_camel_case>Cmd`, for example `AddNumsCmd`. It will add two numbers.
3. Look for the `AppCmd` message declaration and add the new command:
    ...
    message AppCmd {
        oneof type {
            PinSetCmd pin_set = 1;
            // <other commands>
            AddNumsCmd add_nums = 2;
        }
    }
    ```
    - The number assigned to `add_nums` is a [field number](https://protobuf.dev/programming-guides/proto3/#assigning), **not** a value. This number must be unique.
    - Always assign a field number of the previous command (in our case `pin_set`) plus one.
4. Declare `AddNumsCmd` below `AppCmd` and all other application commands:
    ```protobuf { .copy }
    // ------------------------------- AddNumsCmd
    message AddNumsCmd {
        int32 a_in = 1;
    ...
    // =======================================================================
    ```
    - `AddNumsCmd` may have some fields (e.g. the `a_in`) — see [available types](https://protobuf.dev/programming-guides/proto3/#scalar) in the Protobuf documentation (it is also valid to have no fields, like in `GetGpoCmd`). Again, use appropriate [field number](https://protobuf.dev/programming-guides/proto3/#assigning).
5. Look for the `AppResp` message declaration and add the new response `AddNumsResp`:
    ```proto { hl_lines="7" }
    // =============================== AppResp ===============================
    message AppResp {
        ...
        oneof type {
            ...
            // <other responses>
            AddNumsResp add_nums = 3;
        }
    }
    ```
    - The number assigned to `add_nums` is a [field number](https://protobuf.dev/programming-guides/proto3/#assigning), **not** a value. This number must be unique.
    - Always assign a field number of the previous response (in our case `pin_set`) plus one.
6. Declare `AddNumsResp` below `AppResp` and all other application responses, along with `AddNumsRespCode` enum (for result codes):
    ```proto { .copy }
    // ------------------------------- AddNumsResp
    message AddNumsResp {
        AddNumsRespCode res_code = 1;
        int32 c_out = 2;
    }
    enum AddNumsRespCode {
        ADD_NUMS_RESP_CODE_UNSPECIFIED = 0;
        ADD_NUMS_RESP_CODE_OK = 1;
        ADD_NUMS_RESP_CODE_ERROR = 2;
        // other optional result codes
    }
    // =======================================================================
    ```
    - Always declare the `res_code` field — that way, error reporting is unified across all responses.
    - `AddNumsRespCode`: always declare the `*_UNSPECIFIED` value (required by Protobuf) and the `*_OK` value to indicate success. The `*_ERROR` value is not mandatory, but recommended. Additionally, you can declare other specific error codes (see other result code enums for inspiration).
    - `AddNumsResp` may have some fields (e.g. the `c_out`) — see [available types](https://protobuf.dev/programming-guides/proto3/#scalar) in the Protobuf documentation (it is also valid to have no fields, like in `GetGpoCmd`). Again, use appropriate [field number](https://protobuf.dev/programming-guides/proto3/#assigning).
7. Depending on the types you used for `AddNumsCmd` or `AddNumsResp` fields, you may need to add some constraints in the `protobuf/usb_devkit_messages.options` file:
...
1. Create a new header file `add_nums.h` for the command callback function in:
    - `firmware/Inc/app_cmd/`, if it's an application command (our case),
    - `firmware/Inc/raw_cmd/`, if it's a raw command.
2. The contents of `add_nums.h` should look like this:
    ```c { .copy }
    #ifndef ADD_NUMS_H
    #define ADD_NUMS_H

    // This is where the messages are declared.
    #include "usb_devkit_messages.pb.h"

    void add_nums(const AddNumsCmd *cmd, AppResp *resp);

    #endif  // ADD_NUMS_H
    ```
3. Create a new source file `add_nums.c` for the command callback function in: 
    ...
    target_sources(${CMAKE_PROJECT_NAME} PRIVATE
        ...
        ${CMAKE_CURRENT_SOURCE_DIR}/Src/app_cmd/add_nums.c
    )
    ```
4. Implement the command callback function — the contents of `add_nums.c` should look like this:
    ```c { .copy }
    #include "app_cmd/add_nums.h"

    #include "libtropic.h"        // Include if needed.
    #include "libtropic_common.h" // Include if needed.
    #include "main.h"             // Include if needed.
    #include "usb_devkit_messages.pb.h"

    void add_nums(const AddNumsCmd *cmd, AppResp *resp)
    {
        // Add the numbers.
        resp->type.add_nums.c_out = cmd->a_in + cmd->b_in;
        // Set result code.
        resp->type.add_nums.res_code = ADD_NUMS_RESP_CODE_OK;
    }
    ```
5. Call `add_nums()` in:
    ...
    ```c { hl_lines="1 7-10" }
    #include "app_cmd/add_nums.h" // Put it to the top of the file.

    void process_app_cmd(const UsbDevkitCmd *cmd, UsbDevkitResp *resp)
    {
        switch (cmd->type.app.which_type) {
            // <other cases>
            case AppCmd_add_nums_tag:
                resp->type.app.which_type = AppResp_add_nums_tag;
                add_nums(&cmd->type.app.type.add_nums, &resp->type.app);
                break;
...
6. Re-build the firmware in `firmware/build/` to validate all changes (no need to run `cmake` again, just run the generator, e.g. `make`).
...
1. Create a new file `add_nums.py` in `tropic01_util_app/commands/`. In it, we will implement the CLI command that will issue the `AddNumsCmd` USB DevKit application command.
2. At the top of `add_nums.py`, handle imports:
    ```py { .copy }
    ...
    from ..command_core import AppCommandSender, CliCommandSpec
    from protobuf.generated import usb_devkit_messages_pb2 as pb
    ```
3. In the end of `add_nums.py`, create a new instance of `CliCommandSpec` — this defines some basic properties of the CLI command:
    ```py { .copy }
    ADD_NUMS_SPEC = CliCommandSpec(
        name="add-nums",
        help_text="Adds two integers.",  # Short help text.
        description="Adds two 32-bit integers.",  # Detailed description.
...
4. In the middle of `add_nums.py`, implement `add_arguments()` callback:
    ```py { .copy }
    ...
    ```
5. In the middle of `add_nums.py`, implement `execute()` callback:
    ```py { .copy }
    def execute(args: argparse.Namespace,
                app_cmd_sender: AppCommandSender) -> int:
        # Construct the AddNumsCmd to send.
        app_cmd = pb.AppCmd()
        app_cmd.add_nums.a_in = args.a
        app_cmd.add_nums.b_in = args.b
        
        # Send the command.
        app_resp = app_cmd_sender.send(
            app_cmd=app_cmd,
            expected_resp_type="add_nums"
        )

        # Check the result code and print result.
        res_code = app_resp.add_nums.res_code
        print(f"{ADD_NUMS_SPEC.name} result: {pb.AddNumsRespCode.Name(res_code)}")
        if res_code == pb.ADD_NUMS_RESP_CODE_OK:
            print(f"{ADD_NUMS_SPEC.name} addition: {app_resp.add_nums.c_out}")
            return 0
        return 1
    ```
6. In `tropic01_util_app/commands/__init__.py`, register the new CLI command:
    ...
    from .add_nums import ADD_NUMS_SPEC

    # Add new commands here:
    COMMAND_SPECS = {
        # <other commands>
        ADD_NUMS_SPEC.name: ADD_NUMS_SPEC
    }
    ```