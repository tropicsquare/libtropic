...
- [How to use logging macros](#how-to-use-logging-macros)
...
**Important:** Avoid passing function calls as macro arguments (except for simple formatting helpers like `lt_ret_verbose` or `strerror`).
...
*Note: There are also macros used for assertion. These are used in functional tests.*