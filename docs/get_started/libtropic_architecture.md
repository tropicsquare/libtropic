...
5. *L3 Layer API*. Functions called by the *Libtropic Public API* during *Encrypted Communication*. Because the L3 Layer requires some cryptographic functionality to e.g. decrypt incoming L3 Result packets from TROPIC01, it uses a *CAL* (Crypto Abstraction Layer) to get the needed functionality provided by a *CFP* (Cryptographic Functionality Provider). CFP can either be a cryptographic library (e.g. MbedTLS) or a cryptographic HW accelerator. CAL is declared in some of the header files inside `src/` and specific CAL implementations for each CFP exist in `cal/`. It is assumed that only one instance of CAL is used for one Libtropic build.
