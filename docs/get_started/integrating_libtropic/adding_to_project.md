...
# By default, libtropic does not link crypto backend (so it can be built as a
# static library).
# Instead, the user has to:
# 1. choose one of the supported crypto backends (e.g. MbedTLS v4.0.0):
set(LT_CRYPTO_HAL "mbedtls_v4")
# 2. add the `hal/crypto/` subdirectory, which provides the crypto backend HAL
...
# 4. and finally link the cryptographic backend (provided by the consumer) to
#    the 'tropic' target:
