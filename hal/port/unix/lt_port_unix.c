...
lt_ret_t lt_port_init(lt_handle_t *h)
{
    ...
    LOG_OUT("SPI speed: %d\n",   ((spi_def_unix_t*)(h->device))->spi_speed);
    LOG_OUT("SPI device: %s\n",  ((spi_def_unix_t*)(h->device))->spi_dev);
    LOG_OUT("GPIO device: %s\n", ((spi_def_unix_t*)(h->device))->gpio_dev);
    LOG_OUT("GPIO CS pin: %d\n", ((spi_def_unix_t*)(h->device))->gpio_cs_num);

    speed = ((spi_def_unix_t*)(h->device))->spi_speed;
    s.mode = SPI_MODE_0;
    s.fd = open(((spi_def_unix_t*)(h->device))->spi_dev, O_RDWR);
    if (s.fd < 0)
    {
    ...
    s.gpiofd = open(((spi_def_unix_t*)(h->device))->gpio_dev, O_RDWR | O_CLOEXEC);
    if (s.gpiofd < 0)
    {
    ...
    */
    ...
    s.gpioreq.offsets[0] = ((spi_def_unix_t*)(h->device))->gpio_cs_num;
...
lt_ret_t lt_port_spi_transfer (lt_handle_t *h, uint8_t offset, uint16_t tx_data_length, uint32_t timeout)
{
    ...
    struct spi_ioc_transfer spi = {
        .tx_buf        = (unsigned long)h->l2_buff + offset,
        .rx_buf        = (unsigned long)h->l2_buff + offset,
        .len           = tx_data_length,
        .delay_usecs   = 0,
    };
