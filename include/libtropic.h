 ...
 * the rest of TROPIC01. New CPU firmware versions (v2.0.0 and newer) return error when the operation is unsuccessful, but
 * with older firmwares the operation fails silently. Make sure to manually check whether the pairing key was correctly
 ...
 * the rest of TROPIC01. New CPU firmware versions (v2.0.0 and newer) return error when the operation is unsuccessful, but
 * with older firmwares the operation fails silently. Make sure to manually check whether the I-Config was correctly
 * written if operating outside this range. Refer to datasheet for absolute maximum ratings.
 *
 * @param h           Handle for communication with TROPIC01
 * @param addr        Address of a config object
